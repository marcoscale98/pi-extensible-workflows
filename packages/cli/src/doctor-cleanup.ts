...
type ValidatedRole = string | { name: string; model?: string | null | undefined; description?: string | null | undefined; thinking?: ThinkingLevel | null | undefined; tools?: string[] | null | undefined; skills?: string[] | null | undefined; extensions?: string[] | null | undefined; overrideSystemPrompt?: boolean | null | undefined; contextFiles?: string[] | null | undefined };
type ValidatedScheduledOptions = { label: string; cwd: string; tools: string[]; skills?: string[]; extensions?: string[] };
type ValidatedSessionReference = { transport: string; sessionId: string; locator?: unknown };
type ValidatedAgentSetup = { hookNames: string[]; model: ValidatedModel; tools: string[]; cwd: string; resourceSelectors?: unknown };
...
function validateAgentSetup(value: unknown, label: string): asserts value is ValidatedAgentSetup {
  ...
  if (value.resourceSelectors !== undefined) resourceSelectors(value.resourceSelectors, `${label}.resourceSelectors`);
}
...
function validateSnapshot(snapshot: unknown): void {
  ...
  if (snapshot.settingsSources !== undefined) { if (!object(snapshot.settingsSources) || typeof snapshot.settingsSources.concurrency !== "string" || typeof snapshot.settingsSources.modelAliases !== "string" || snapshot.settingsSources.skills !== undefined && typeof snapshot.settingsSources.skills !== "string" || snapshot.settingsSources.extensions !== undefined && typeof snapshot.settingsSources.extensions !== "string" || snapshot.settingsSources.tools !== undefined && typeof snapshot.settingsSources.tools !== "string") throw new Error("Persisted snapshot settings sources are invalid"); }
  const settingsRecord: Record<string, unknown> = snapshot.settings;
  ...
  if (settingsRecord.modelAliases !== undefined) validateModelAliases(settingsRecord.modelAliases);
