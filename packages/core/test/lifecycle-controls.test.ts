...
void test("run lifecycle cancels a pending pause and keeps running (#283)", async () => {
  const states: string[] = [];
  const lifecycle = new RunLifecycle("running", (state, _previous, reason) => { states.push(`${state}:${reason ?? ""}`); });
  await lifecycle.enter();
  await lifecycle.pause();
  assert.equal(lifecycle.state, "pausing");
  let continued = false;
  ...
  await waiting;
  assert.equal(continued, true, "operations blocked by the pending pause continue");
  await lifecycle.leave();
  await lifecycle.leave();
  assert.equal(lifecycle.state, "running", "the cancelled pause does not fire when the active operation finishes");
  assert.deepEqual(states, ["pausing:pause", "running:resume"]);
});
void test("the picker cancels a pending pause while an agent is still active (#283)", async () => {
  const home = mkdtempSync(join(tmpdir(), "pi-extensible-workflows-cancel-pause-"));
  const tools: Array<{ name: string; execute: (...args: unknown[]) => Promise<unknown> }> = [];
  const commands: Array<{ handler: (args: string, ctx: unknown) => Promise<void> }> = [];
  let releaseAgent!: () => void;
  const held = new Promise<void>((resolve) => { releaseAgent = resolve; });
  let started = false;
  const createSession = async (): Promise<TestPiSession> => ({ sessionId: "cancel-pause", sessionFile: "/sessions/cancel-pause.jsonl", messages: [{ role: "assistant", content: [{ type: "text", text: "done" }] }], getSessionStats: () => ({ tokens: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 }, cost: 0 }), prompt: async () => { started = true; await held; }, steer: async () => {}, abort: async () => { releaseAgent(); }, dispose() {} });
  ...
  const context = { cwd: home, hasUI: false, model: { provider: "openai", id: "gpt" }, sessionManager: { getSessionId: () => "session" }, ui: { notify(message: string) { notifications.push(message); } } };
  const running = workflow.execute("id", { name: "cancel-pause", script: "return await agent('work');", foreground: true }, new AbortController().signal, undefined, context);
  await waitForIssue105(() => started);
  const runId = (await listRunIds(home, "session", home))[0];
  assert.ok(runId);
  const store = new RunStore(home, "session", runId, home);
  await contextualWorkflowAction(command, context, runId, (options) => options.includes("Pause") ? "Pause" : undefined);
  assert.equal((await store.load()).run.state, "pausing");
  assert.ok(notifications.some((message) => message.startsWith("Pausing workflow")), notifications.join("\n"));
  assert.equal(notifications.some((message) => message.startsWith("Paused workflow")), false, "a pending pause is not reported as paused");
  let offered: string[] | undefined;
  await contextualWorkflowAction(command, context, runId, (options) => { offered ??= options; return options.includes("Cancel pause") ? "Cancel pause" : undefined; });
  assert.ok(offered?.includes("Cancel pause") && !offered.includes("Resume") && offered.includes("Stop"), offered?.join(", "));
