...
Set `worktree` on `subagents_run` to create a named isolated Git worktree for that run. The default adapter uses the core `RunStore`: the worktree branches from the repository's clean `HEAD`, and a dirty working tree (tracked or untracked non-ignored changes) fails the run with `WORKTREE_FAILED`. The optional global or trusted-project `worktreePostCreateCommand` setting runs its executable and arguments directly, once after a physical checkout, from the launch-relative directory inside the new worktree. Install that external executable separately; missing executables, nonzero exits, spawn failures, and the fixed 60-second timeout fail with `WORKTREE_FAILED` and roll back creation. The setting is captured when the subagent starts, and an untrusted project cannot enable it. The executor runs in the worktree and inspection exposes its path and branch while materialized. Cleanup runs when the agent settles, stops, or the manager reconciles an interrupted record. After successful cleanup, the public and persisted worktree path, branch, and cleanup context are removed; if cleanup fails, all of that metadata is retained for a later retry. A run ID keeps concurrent worktrees separate even when their names are the same.
