...
import { runsDirectory, safePart, sameFilesystemPath, structuralPath, workflowProjectSettingsPath } from "./paths.js";
...
export class RunStore {
  ...
  private async runWorktreePostCreateCommand(command: readonly string[], cwd: string, source?: string): Promise<void> {
    ...
    try {
      ...
      const provenance = source !== undefined && sameFilesystemPath(source, workflowProjectSettingsPath(this.cwd)) ? "trusted project settings" : "global settings";
      let category: string;
