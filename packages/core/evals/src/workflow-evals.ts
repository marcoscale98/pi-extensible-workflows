...
function evalYaml(content: string, source: string): unknown { try { const parsed = parseFrontmatter(`---\n${content.replace(/\r\n?/g, "\n")}\n---\n`); if (parsed.body.trim()) evalValidationError(source, "<document>", "must contain one YAML document"); return parsed.frontmatter; } catch (error) { if (error instanceof Error && error.message.startsWith("YAML ")) throw error; evalValidationError(source, "<document>", `malformed YAML: ${errorText(error)}`); } }
function evalCasesDirectory(): string { const moduleDirectory = dirname(fileURLToPath(import.meta.url)); const candidates: readonly [string, string] = [join(moduleDirectory, "../cases"), join(moduleDirectory, "../../../evals/cases")]; const found = candidates.find((candidate) => existsSync(candidate)); if (!found) evalValidationError(candidates[0], "<document>", "cases directory not found"); return found; }
  ...
  try {
    ...
    if (selection.callIndices.length > 0) {
      ...
      if (judgeProcess.exitCode !== 0 || judgeProcess.error) errors.push("Semantic judge process failed.");
        ...
        } catch (error) { errors.push(`Invalid semantic judge output: ${errorText(error)}`); }
      }
    }
...
export async function runIsolatedProcess(payload: unknown, options: IsolatedProcessOptions): Promise<IsolatedProcessResult> {
  ...
  try {
    ...
    try {
      ...
      return { timedOut, exitCode, processGroupTerminated, stderr, error: `Invalid child JSON: ${errorText(error)}` };
    }
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
